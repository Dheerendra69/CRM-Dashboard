...
router.post("/create2", async (req, res) => {
  ...
  try {
    ...
    // Step 2: Extract rules using custom logic (basic regex fallback here)
    const rules = [];
    ...
    // Note: Replace `localhost:5000` with your actual backend URL in production

    const audience = audienceRes.data.customers || [];

    // Step 4: Save Campaign in required format
